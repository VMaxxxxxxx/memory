1. `xv6`是什么内核架构

   > 宏内核：将进程管理、文件管理、设备驱动、内存管理等都在一个内核空间中实现，`UNIX`常用，模块之间调用简单效率高，但内核代码庞大，一个模块出错可能会影响整个系统崩溃
   >
   > 微内核：将内核功能尽量精简，只保留最基础的部分（进程间通信、调度、内存管理等），其他服务都在用户态进程中，通过消息传递与内核交互，稳定性和拓展性较好，但用户态和内核态切换频繁，性能开销

2. `xv6`的内核源码

   > - 启动与内核初始化
   >
   >   > **`entry.S`**：内核入口（从汇编开始执行），设置栈、跳转到`start`然后跳转到 `main`。
   >   >
   >   > **`start.c`**：内核 C 语言的第一个入口（`main`），负责初始化内核子系统。
   >   >
   >   > **`kernel.ld`**：链接脚本，定义内核在内存中的布局。
   >   >
   >   > **`trampoline.S`**：用户态和内核态切换时的“跳板”代码（切换页表）。
   >   >
   >   > **`kernelvec.S`**：内核的 trap 向量表（中断/异常的入口）。
   >
   > - 进程与调度
   >
   >   > **`proc.c / proc.h`**：进程管理（`fork`, `exit`, `scheduler`, PCB `struct proc`）。
   >   >
   >   > **`swtch.S`**：上下文切换（保存/恢复寄存器）。
   >   >
   >   > **`sysproc.c`**：与进程相关的系统调用实现（`getpid`, `fork`, `exit` 等）。
   >
   > - 内存管理
   >
   >   > **`vm.c`**：虚拟内存管理（页表、映射、页分配）。
   >   >
   >   > **`vmcopyin.c`**：用户态到内核态安全拷贝数据的辅助函数。
   >   >
   >   > **`kalloc.c`**：物理内存分配器（基于空闲链表）。
   >
   > - 系统调用与中断
   >
   >   > **`syscall.c / syscall.h`**：系统调用分发（从系统调用号到函数指针）。
   >   >
   >   > **`sysfile.c`**：与文件系统相关的系统调用（`open`, `read`, `write` 等）。
   >   >
   >   > **`sysproc.c`**：进程相关的系统调用。
   >   >
   >   > **`trap.c`**：中断/异常处理。
   >   >
   >   > **`defs.h`**：函数声明（避免循环依赖）。
   >
   > - 文件系统与`IO`
   >
   >   > **`fs.c / fs.h`**：文件系统核心（`inode`、目录、超级块）。
   >   >
   >   > **`file.c / file.h`**：文件描述符层（管理 `struct file`，抽象设备和普通文件）。
   >   >
   >   > **`log.c`**：日志（写时复制日志，保证文件系统一致性）。
   >   >
   >   > **`bio.c / buf.h`**：缓冲区缓存（block cache，管理磁盘块缓冲）。
   >   >
   >   > **`pipe.c`**：管道实现（进程间通信）。
   >   >
   >   > **`sleeplock.c / sleeplock.h`**：睡眠锁，用于文件系统。
   >   >
   >   > **`spinlock.c / spinlock.h`**：自旋锁。
   >
   > - 设备驱动
   >
   >   > **`uart.c`**：UART 串口驱动（控制台输入输出）。
   >   >
   >   > **`virtio_disk.c / virtio.h`**：VirtIO 磁盘驱动（QEMU 模拟磁盘）。
   >   >
   >   > **`ramdisk.c`**：RAM 磁盘驱动（实验用）。
   >   >
   >   > **`plic.c`**：中断控制器 PLIC 驱动。
   >   >
   >   > **`console.c`**：控制台设备（用 UART 实现）。

3. `xv6`的启动流程

   > ```markdown
   > 上电 / 复位
   >    ↓
   > M 模式 (OpenSBI)
   >    - 初始化硬件
   >    - 进入 S 模式
   >    ↓
   > S 模式 (xv6 内核)
   >    entry.S → start.c → main()
   >    - 初始化子系统 (内存/进程/文件系统/驱动)
   >    - 创建第一个用户进程 init
   >    ↓
   > U 模式
   >    initcode.S (调用 exec("/init"))
   >    ↓
   > 运行 /init
   >    - 启动 shell (sh)
   >    ↓
   > 用户可以输入命令
   > ```
   >
   > **硬件上电 → 进入 M 模式 (Machine mode)**
   >
   > - `RISC-V`上电时，会初始化自己，运行一个存储在只读内存中的引导加载程序
   > - 引导加载程序将`xv6`内核加载到内核上
   >
   > **进入内核入口 → entry.S**
   >
   > - `CPU`从 `kernel/entry.S`开始运行`xv6`，为 xv6 设置好初始执行环境。
   > - `xv6`启动时，页式硬件处于禁用模式，虚拟地址直接映射到物理内存
   > - 设置内核栈指针，这样才能运行`C`代码。
   > - 跳转到 `start.c` ，先执行一些仅在机器模式下允许的配置，然后切换到管理模式，将 `main()`的地址写入寄存器`empc`将返回地址设为`main`，通过向页表寄存器`satp`中写`0`来禁止管理模式下的虚拟地址转换，将中断和异常委托给管理模式。
   > - 然后对时钟芯片编程产生定时器中断，调用`mret`返回到管理模式，由于之前将返回值设置为`main`，因此`mret`后面会返回到`main`函数，并以管理模式运行
   >
   > **内核初始化 (main.c)**
   >
   > - `main()` 调用一系列子系统初始化：
   >   - `consoleinit()`：初始化控制台。
   >   - `printfinit()`：调试打印。
   >   - `kinit()`：物理内存分配器。
   >   - `kvminit()`：内核页表。
   >   - `trapinithart()`：设置中断向量表。
   >   - `plicinit()`：中断控制器初始化。
   >   - `binit()`, `fileinit()`, `iinit()`, `virtio_disk_init()`：文件系统 & 块设备。
   >   - `procinit()`：进程表初始化。
   >
   > **创建第一个进程 (userinit)**
   >
   > - `userinit()` 创建 **第一个用户进程 `init`**。
   > - 给它分配一个进程控制块（PCB）、用户页表。
   > - 加载 `initcode.S`（用户空间的第一段小程序）。
   > - 把进程状态设为 RUNNABLE。
   >
   > **调度器 (scheduler)**
   >
   > - `main()` 最后调用 `scheduler()`，开始调度循环。
   > - `scheduler()` 找到第一个可运行进程（就是 `init`），进行上下文切换。
   > - CPU 切换到 U 模式，运行 `initcode.S`。
   >
   > **进入用户态 → initcode.S**
   >
   > - `initcode.S` 是一个很小的用户程序，主要任务是调用 `exec("/init")`。
   > - 这样就把真正的用户程序 `/init` 从文件系统加载到内存。
   >
   > **运行 `/init` 用户程序**
   >
   > - `/init` 是一个用户空间 C 程序，它会执行：
   >   - 打开控制台。
   >   - 创建第一个 shell (`sh`)。
   > - 至此，系统完全启动，用户可以在 shell 里运行命令。

4. 页表初始化的时机

   > 1. 启动时没有页表，使用物理寻址
   > 2. 在entry.S阶段，建立一个临时页表，只映射内核需要的部分（KERNBASE、trampoline和stack的必要区域）
   > 3. kvminit阶段，建立完整的内核页表，替换临时页表
   > 4. 进程创建时，再为每个用户进程建立用户态页表
   >
   > > - satp寄存器用于存放根页表的物理地址，每个页表项PTE包含下一级页表的物理地址，缺一个就引发页面故障异常
   > >
   > > - 不同的cpu有自己的satp，即可运行不同的进程
   > >
   > > - RISC-V硬件支持
   > >
   > >   > 提供了硬件页表支持，包括多级页表和地址转换机制SV39
   > >
   > > - xv6运行在RISC-V架构的CPU上，并使用SV39分页机制来管理虚拟地址空间
   > >
   > > - RISC-V指令使用的是`虚拟地址`，机器的`RAM或物理内存`是由物理地址索引的，RISC-V `页表硬件`通过将每个虚拟地址映射到物理地址。
   > >
   > > - SV39：只使用64位虚拟地址的低39位，高25不用。
   > >
   > > - 逻辑上，RISC-V的页表是由`2^27`个页表项`pte`组成的数组。每个`pte`包含一个44位的物理页码`PPN`和一些标志位。分页硬件利用虚拟地址39位中的27位索引页表，找到虚拟地址对应的一个`pte`，这个`pte`的`ppn`包含44位，再利用虚拟地址剩下的12位页内偏移生成一个56位的物理地址。
   > >
   > > - 页表使`os`能以4096字节的对齐粒度控制虚拟地址到物理地址的转换，即一页
   > >
   > > - 最大虚拟地址空间：`2^39字节=512GB`
   > >
   > > - 最大物理地址空间：`2^56字节`
   > >
   > > - 实际上，转换分成三步，虚拟地址的`L2`用于索引第一级页表(根页表)，`L1`用于索引第二级页表，`L0`用于索引第三级页表(叶子页表)

5. 用户页表和内核页表

   > risc-v的xv6，每个用户进程都有自己独立的用户页表，包含两部分：
   >
   > - 用户空间部分：映射自己的代码、数据、堆、栈，页表项的权限设置为U=1，用户态可以访问
   > - 内核空间部分：并不是把整个内核页表复制一份给用户，而是每个用户页表都会额外映射到一些固定的内核区（trampoline page用于用户内核态切换，trapframe保存寄存器现场），但完整的内核映射存在于内核自己的全局页表中，当用户触发陷入时切换到内核态，才会将satp指向内核页表，访问整个内核

6. 物理地址和虚拟地址

   > - `CPU`访问的真实硬件内存地址，没有`MMU`的系统中，程序访问的就是物理地址
   > - `CPU`指令中使用的地址，不直接访问内存，通过`MMU`转换成物理地址，每个进程看到的是独立的虚拟地址空间，由**页表**定义虚拟地址到物理地址的映射
   >   - `RISC-V`使用`Sv39`分页机制，对`64`位宽的地址只使用低`39`位，为每个进程维护一个独立的页表，并且所有进程共享同一个内核页表
   >   - `xv6`进程的地址空间分布：指令、全局变量、栈、堆、**`trapframe`**、**`trampoline`**
   >   - ![image-20250818172202649](C:\Users\19855\AppData\Roaming\Typora\typora-user-images\image-20250818172202649.png)
   >   - 进程状态：未使用、就绪、运行、阻塞、退出

7. `xv6`的进程

   > - `xv6`使用页表为每个进程提供自己的地址空间，`RISC-V`页表将虚拟地址转换成物理地址
   > - 每个进程都有一个**执行线程**来执行进程的指令，内核通过不断挂起当前运行的线程，恢复另一个进程的线程来实现调度
   > - 线程的大部分状态（本地变量、函数调用返回地址）存储在线程的栈上
   > - 每个进程有两个栈区：**用户栈、内核栈**；当执行用户指令的时候，内核栈为空；进程陷入内核时，内核代码在进程的内核栈上执行，此时其用户栈中仍然保存有数据，只是不活跃
   > - 进程通过执行`RISC-V`的**`ecall`**指令进行系统调用，该指令能够提升硬件特权级别，将程序计数器更改为内核定义的入口点，入口点的代码切换到内核栈，执行实现系统调用的内核指令；
   > - 系统调用完成，内核切换回用户栈，通过**`sret`**指令返回用户空间，降低硬件特权级别，在指令结束时恢复执行用户指令

8. ==**`理解xv6系统调用流程`**==

   > - 用户态调用封装的函数
   >
   >   > 调用的是库函数，被关联在`user/usys.S`中，对应一个系统调用号，并执行一个`ecall`指令
   >
   > - 执行`ecall`，`trap`陷入到内核
   >
   >   > `ecall`指令触发陷入`trap`， `CPU`从用户模式切换到管理模式，硬件会保存当前用户态的`pc`，跳转到`trap`向量`kernelvec.S`，保存寄存器到`trapframe`，跳转到`kernel/trap.c/usertrap()`
   >
   > - 内核处理系统调用
   >
   >   > `usertrap`检查`trap`原因，如果是系统调用，调用`syscall()`，会从`a7`寄存器中取出系统调用号，根据系统调用号，在`syscalls[]`这个注册表中找到对应的内核函数
   >
   > - 执行系统调用的具体实现
   >
   >   > 利用一些辅助函数，可以从`trapframe`中取出参数，`argint()、argaddr()`
   >
   > - 返回用户态
   >
   >   > 将系统调用函数返回值写入`a0`寄存器，回到`syscall()`、`usertrap()`、`usertrapret()`，切换到用户页表，将`CPU`状态恢复成用户态，执行`sret`指令，回到用户程序
   >
   > - ```markdown
   >   用户程序: write(fd, "hello", 5)
   >      ↓
   >   usys.S: a7=SYS_write; ecall
   >      ↓
   >   硬件: U-mode → S-mode
   >      ↓
   >   kernelvec.S → usertrap()
   >      ↓
   >   syscall()
   >      ↓
   >   sys_write() in sysfile.c
   >      ↓
   >   内核 write() → 文件系统/驱动
   >      ↓
   >   返回值放在 a0
   >      ↓
   >   usertrapret() → sret
   >      ↓
   >   回到用户态，write() 返回
   >   ```

9. ==**`基于用户态和内核态的数据传递和访问机制`**==

   > - 用户态传递参数给内核（系统调用号、系统调用参数）
   >   - `RISC-V`规定，系统调用参数放在`a0-a5`寄存器中，系统调用号在`a7`
   >   - 在`xv6`内核中，`syscall()`会从`trapframe`中取出这些寄存器，调用`argint()`、`argaddr()`得到参数：`argint(0, &fd); argaddr(1, &p);`
   > - 内核访问、修改用户态数据（用户态数据在虚拟地址空间中，而内核空间页表不能直接访问）
   >   - 用户传递的指针，是用户虚拟地址地址，内核不能直接当做内核地址用，因为用户空间和内核空间的页表不一致，`xv6`使用`copyin`、`copyout`完成数据搬运：`copyin(pagetable, dst, srcva, len);//从用户虚拟地址拷贝到内核缓冲区`，`copyout(pagetable, dstva, src, len);//将数据写回用户虚拟地址`
   > - 在系统调用执行结束，把返回值写回`a0`寄存器，用户态函数就能拿到结果

10. ==**`拓展实现trace、sysinfo等系统调用`**==

   > - 系统调用：操作系统通过`CPU`的特权模式将用户态和内核态隔离，保证了用户程序无法直接访问内核资源，规避了误操作和恶意行为，系统调用作为用户程序和内核交互的桥梁，运行用户程序通过受控的接口向内核请求服务，保证系统的安全性和稳定性
   >
   > - 系统调用机制：通过库函数间接发起系统调用，会触发软件中断或陷阱，引发`CPU`起换到内核态，内核根据系统调用号来确定要执行的服务，完成服务后，将结果或错误码写入用户程序的寄存器中，然后返回到用户态继续执行程序
   >
   > - 由于内核和用户进程的页表不同，寄存器也不互通，参数无法直接通过c语言参数的形式传递，因此使用`argint()`等系列函数，从进程的`trapframe`中读取用户进程寄存器中的参数
   >
   > - `trace 系统调用：需要记录系统调用号、参数 → 内核打印 → 返回给用户。`
   >
   >   - 对`trace 32 grep hello README`指令，利用`32`设置按位计算的掩码，在执行`grep hello README`时，将掩码对应跟踪的系统调用过程进行输出
   >
   >   - ```c
   >     // trace 系统调用流程
   >     trace.c				// trace(atoi(argv[1])执行系统调用
   >     user/user.h			// 用户态程序函数trace(int mask)声明
   >     user/usys.S			// usys.pl:entry("trace");会生成usys.S，提供trace系统调用的接口，填入a7寄存器一个系统调用号，然后执行ecall陷入内核
   >     kernel/trampoline.S/uservec		// trap向量入口，保存用户上下文，切换内核栈
   >     kernel/trap.c/usertrap()		// 判断trap类型（syscall、外部中断、异常），如果是系统调用，需要调用syscall
   >     kernel/syscall.c	// 到达内核统一syscall()，所有系统调用都会在此处理，进行分发
   >     kernel/syscall.c	// 根据跳板传进来的a7系统调用号，查询syscall.h和syscall[]注册表中对应的内核函数映射关系，调用具体内核函数实现
   >     kernel/sysproc.c	// 处理具体的sys_trace()函数逻辑，使用argint、argaddr获取参数
   >     kernel/syscall.c	// 在a0写入系统调用函数返回值
   >     kernel/trap.c/usertrapret()	// 准备返回用户态，恢复用户态上下文，切换用户页表
   >     kernel/trampoline.S/userret	// 执行sret指令，真正切换到用户态，继续执行用户程序
   >     ```
   >
   > - `sysinfo 系统调用：需要把内核里的内存使用信息、进程数拷贝到用户态的结构体里。`
   >
   >   - `sinfo(&info);`在用户态调用
   >
   >   - ```c
   >       uint64 dstaddr;		//  获取用户虚拟地址，第一个实参info，然后在函数内形参进行填充
   >       argaddr(0, &dstaddr); 
   >       // 从内核空间拷贝数据到用户空间
   >       if(copyout(myproc()->pagetable, dstaddr, (char*)&info, sizeof info) < 0) return -1;
   >     ```

11. ==`基于共享页表机制改写 copyin / copyinstr，简化系统调用数据传输流程`==

    > 

12. ==`分析中断处理流程`==

    > 

13. ==`基于时钟中断实现alarm系统调用`==

    > 

14. ==`支持用户态触发定时任务`==

    > 

15. risc-v的概念、arm的概念

16. xv6运行在哪里（qemu模拟的硬件环境）

17. risc-v几级流水（取值、译码、执行、访存、写回），会不会出现pc减的情况（分支、异常、中断的指令跳转），pc正常情况每次加多少（4或8）

18. 用户页表的地址（proc.h/proc.pagetable）、内核页表的地址（proc.trapframe.kernel_satp）

19. trampoline的作用：存放着一些共享的汇编代码，映射着kernel/trampoline.S，用于进程进出内核时保存恢复寄存器、切换栈、页表等

20. 为什么要合并内核和用户页表，合并不会有冲突吗

21. 为什么有虚拟内存（内存隔离、更大的地址空间、内存映射和文件共享、内存管理、交换和页表置换）

22. 为什么可以使所有虚拟内存之和超过物理内存（核心是缺页中断机制，实现了写时复制、懒分配、磁盘交换等）

23. 虚拟地址空间大小取决于cpu的寄存器宽度和操作系统设计，32位cpu上限2^32=4GB，64位理论上16EB，但一般实现低48位=256TB，或57位，实际可寻址空间远小于理论值，risc-v的sv39是39位虚拟地址，512GB

24. 操作系统内存管理有哪些模块：进程管理、虚拟内存管理、内存分配与释放、页面置换、共享内存、内存映射文件、页面回收与垃圾收集、内存监控与性能优化

25. xv6没有实现页面置换，避免复杂的swap机制，保持内核最小化

26. xv6是如何分配内存的，用户态通过malloc申请，维护了内存池，无法提供合适内存时（当空闲链表中没有足够大块满足malloc请求时，通过sbrk向内核请求更多内存，然后把新内存以块的形式加入空闲量表），调用sbrk向操作系统申请新的拓展堆内存（在页表中做标记、提供虚拟内存、通过kalloc申请物理内存并做mappages映射到虚拟内存，返回虚拟内存），空闲的物理内存通过链表维护，kalloc在空闲链表上获取内存`user->syscall->proc.c->vm.c:uvmalloc->kalloc->freelist`

27. xv6的栈（xv6为每个栈分配两页内存，4KB*2，一页用于实际栈，另一个防止溢出，函数调用会在栈上产生栈帧，都保存有返回地址、寄存器、局部变量等）

28. xv6不支持多线程，Linux中多线程每个线程都有自己私有的线程栈（线程释放可以回收，也可以设置线程栈池，方便快速分配栈内存，重复使用）

29. 频繁缺页异常影响性能，避免措施（调整swappiness、关闭非必要后台服务、调整页面大小）

30. 页表的作用（地址映射、内存隔离、虚拟内存管理、内存保护）

31. 三级页表对比一级页表的优劣

    > - 单级页表每个进程都需要维护一个完整的页表，其大小是取决于虚拟地址空间的大小，而多级页表按需分配，仅为实际访问的页面分配页表，大大减小内存开销
    > - 多级页表能够通过分级索引，使得页表可以随着进程需求增长，而不是一次性分配整个页表结构，并且还能减少对大块连续物理内存的需求，提升内存管理的灵活性
    > - 多级页表需要多次进行内存访问，增加了地址转换的开销，但可以通过转换后备缓冲区TLB缓存页表项来缓解
    > - 多级页表额外增加了页表的管理逻辑，如分配和释放页表页、地址解析算法，使实现复杂度增加

32. 快表的概念：页表的一种缓存，在虚拟地址转换成物理地址时，优先访问快表，减少访存次数

33. 快表原理：局部性原理（时间局部性、空间局部性）

34. 用户态分配的内存可以直接被内核态访问到吗（理论上用户态和内核态程序，访问的都是自己的虚拟地址空间，无法感知其他程序的虚拟地址空间，但内核态程序的虚拟地址空间采用直接映射，虚拟地址就是物理地址，如果知道用户态分配的内存对应的物理地址，内核态程序可以访问用户程序的数据）

35. 缺页异常如果内存不够，发生什么（xv6在缺页时，触发panic，其他操作系统会采取措施来获取物理内存（换出到磁盘、触发内存溢出OOM机制））

36. 堆和栈的区别（分配方式、管理方式、生命周期、空间大小、访问速度、用途）

37. malloc的内存池找不到合适内存块，怎么做

38. xv6有进程（独立的虚拟地址空间，独立的用户栈、内核栈、进程状态（runnable、running、sleeping），通过fork、exec、wait、exit管理线程，调度的基本单位），没有线程（模拟线程只能使用fork创建多进程，但这不共享数据）、没有协程

39. xv6系统调用的流程（系统调用是以函数的形式暴露给进程的）

40. ecall的作用（关中断、切换特权模型、设置寄存器、跳转trampoline.S）

41. xv6的上下文切换（进程切换）包含哪些内容（通用寄存器、栈底指针sp）

42. trampoline.S的作用是什么（包含了两个状态切换函数，保存用户态的通用寄存器状态、载入之前保存的内核态通用寄存器状态，切换页表，刷新快表，切换函数栈），其映射到了每个进程虚拟地址空间的相同地址上

43. xv6有哪些锁（自旋锁spinlock.c（test_and_set原子指令实现）、睡眠锁sleeplocl.c（自旋锁+循环+sleep实现）），怎么实现的，各自的使用场景是什么（自旋锁使用与锁持有时间短的场景、睡眠锁适合锁持有时间长的场景）

44. 如何降低锁竞争（读写锁、无锁数据结构、锁拆分）

45. 如何降低锁争用

46. 如何处理死锁问题（预防死锁、避免死锁、死锁检测与恢复、并发库工具、良好设计、测试与代码审查）

47. 为什么xv6的单个文件限制是168k，而不是256这种2的n次方（xv6文件系统的inode在索引文件数据时混合了直接和间接方法，有12个直接索引和1个间接索引，每个直接索引指向一个数据块，共直接索引了12kb，间接索引也指向一个数据块，但数据块上存储256个二次索引，每个二次索引指向一个数据块，共间接索引了256kb，计256kb）

48. 为什么操作系统里要设置文件缓存（减少访问磁盘IO的次数，提升访问速度）

49. 段错误的排查解决方式（使用gdb对段错误产生的核心转储文件进行调试，在发生段错误的位置，使用bt查看调用栈，定位产生段错误的函数，再阅读源码解决问题）

50. 哪些情况会产生段错误（数组越界、野指针、栈溢出、对只读内存的写、不正确的类型转换、内存映射文件错误）

51. xv6的前十行是汇编指令，进行配置c语言环境，跳转去执行其他初始化操作

52. xv6的SV39页表地址转换

    > - 一个64位的虚拟地址，只用到了低39位，高25位不用，低39位中的低12位作为偏移量，高27位分3级9位标识物理页码
    > - 页表以三级的树形结构存储在物理内存中，数的根是一个4096字节的页表页，包含了512个页表项PTE，每个PTE中包含该树下一级页表页的物理地址，每一个页表页都包含了512个页表项PTE
    > - 分页硬件使用27位的前9位在根页表中选择PTE，中间9位在下一级页表中选择PTE，最后9位选择最终的PTE，最终的PTE里存放的就是物理页地址（44位），拼接上12位的页内偏移，得到物理地址（56位，上限512GB）
    > - 如果三级PTE有任何一个不存在，页式硬件发生缺页中断，让内核处理
    > - 如果在一级页表目录中只使用条目0，那剩下的条目及其中间页面不分配，三级设计只分配3*4096字节
    > - 使用快表减少从物理内存加载PTE开销，讲页表条目缓存在TLB中
    > - PTE包含标志位，存在、读、写、执行、用户模式运行访问等
    > - 内核必须将根页表页的物理地址写入satp寄存器，每个cpu有自己的satp，使用自己的satp指向的页表转换后续指令生成的所有地址（虚拟地址）

53. 为什么要采用直接映射

54. 内核地址空间中，低地址部分是IO设备区，然后从KERNBASE开始做直接映射（代码段、数据段），空闲物理内存可分配给内核和用户的物理内存，PHYSTOP上面有多个内核栈（每个进程在内核态有一个栈和保护页），最高处是映射了一个trampoline跳板页（用于处理内核和用户态的切换）

55. 内核自己没有独立的全局栈，而是复用每个进程的内核栈

56. 页表的walk（虚拟地址查找PTE）和mappages（为新映射装在PTE）函数

57. copyout（复制数据到用户虚拟地址空间）、copyin（从用户虚拟地址空间复制）

58. 内核末尾到PHYSTOP之间的物理内存用于运行时分配，使用链表的数据结构将空闲页面记录下来